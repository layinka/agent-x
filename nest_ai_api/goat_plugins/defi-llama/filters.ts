...
export function filterPoolByChain(pools: any[], chainName: string): any[] {
  return pools.filter((pool) => pool.chain === chainName);
}
...
export function filterPoolByExposure(pools: any[], exposure: string): any[] {
  return pools.filter((pool) => pool.exposure === exposure);
}
...
export function filterPoolByAllField(
  ...
  return pools.filter((pool) => {
    return (
      (chainName === undefined || pool.chain === chainName) &&
      ...
      (exposure === undefined || pool.exposure === exposure) &&
